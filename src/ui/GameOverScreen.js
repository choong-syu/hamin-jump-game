export function showGameOver(overlay,score,best,level,playerName,records,reason="구름 아래로\n살포시 착지!",racePlayers=[],coinsEarned=0) {
  overlay.classList.remove("start-background");
  const rows=records.slice(0,5).map((record,index)=>`<li><strong>${index+1}</strong><span>${record.name?`${escapeHtml(record.name)} · `:""}Lv.${record.level}</span><strong>${record.score.toLocaleString()}</strong></li>`).join("");
