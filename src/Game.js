...
export class Game {
  ...
  gameOver(){this.state=GameState.GAME_OVER;this.player.state=PlayerState.DEAD;this.best=Math.max(this.best,this.score);this.save(STORAGE_KEYS.bestScore,this.best);this.addRecord();this.particles.burst(this.player.x+40,760,"#fff",22,150);this.audio.tone(120,.5,"sawtooth");showGameOver(this.overlay,this.score,this.best,this.level,this.playerName,this.records);}
  render() {
    ...
    if(this.state===GameState.PLAYING||this.state===GameState.PAUSED)drawHUD(this.ctx,this.score,Math.max(this.best,this.score),this.level);
  ...
  drawBackground() {
    ...
    this.ctx.globalAlpha=.32;this.ctx.fillStyle="#fff";for(let i=0;i<6;i++){const x=(i*113+this.distance*.05)%590-60,y=130+i*112;this.ctx.beginPath();this.ctx.ellipse(x,y,58,20,0,0,Math.PI*2);this.ctx.fill();}
